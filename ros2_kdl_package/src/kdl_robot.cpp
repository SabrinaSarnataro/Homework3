...
void KDLRobot::update(std::vector<double> _jnt_values, std::vector<double> _jnt_vel, std::vector<double> _jnt_eff)
{
    int err;
    updateJnts(_jnt_values, _jnt_vel, _jnt_eff);
...
void KDLRobot::updateJnts(std::vector<double> _jnt_pos, std::vector<double> _jnt_vel, std::vector<double> _jnt_eff)
{
    for (unsigned int i = 0; i < n_; i++)
    {
        jntArray_(i) = _jnt_pos[i];
        jntVel_(i) = _jnt_vel[i];
        jntEff_(i) = _jnt_eff[i];
    }
}
...
{
    f_F_ee_ = _f_F_ee;
    this->update(toStdVector(this->jntArray_.data), toStdVector(this->jntVel_.data), toStdVector(this->jntEff_.data));
}
